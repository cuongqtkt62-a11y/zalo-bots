...
- Chủ đề gợi ý cho bài này: "{suggestedTopic}"`;
    ...
    try {
      ...
      // Fallback bám sát mục đích nhóm, ngắn gọn, KHÔNG chào hỏi — đúng phong cách trợ lý Cường
      const purposeLabel = groupPurpose || groupName || 'phát triển bản thân';

      if (timeOfDay.includes('sáng') || timeOfDay.includes('8')) {
        const fallbacks = [
          { post: `🌿 Muốn đi xa trong ${purposeLabel}, hãy bắt đầu bằng 1 thói quen nhỏ mỗi sáng.\n\nKhông cần hoàn hảo — chỉ cần kiên trì mỗi ngày 1%.\n\nHôm nay cả nhà đặt mục tiêu gì cho mình? 🎯`, quote: 'Kiên trì mỗi ngày, thành công sẽ đến', cursive_quote: 'Mỗi ngày tiến một bước để', highlight_quote: 'KIÊN TRÌ TẠO THÀNH CÔNG' },
          { post: `✨ Người thành công trong ${purposeLabel} không phải giỏi nhất — mà là người không bỏ cuộc.\n\nMỗi sáng thức dậy, hãy tự hỏi: "Hôm nay mình sẽ tiến thêm được bước nào?"\n\nCả nhà đang tập trung vào điều gì? 🎯`, quote: 'Không bỏ cuộc là đã thắng một nửa', cursive_quote: 'Kiên định mỗi ngày để', highlight_quote: 'KHÔNG BAO GIỜ BỎ CUỘC' },
          { post: `☕️ Một nguyên tắc em luôn áp dụng trong ${purposeLabel}:\n\nLàm việc quan trọng nhất TRƯỚC — khi đầu óc còn tỉnh táo nhất.\n\nĐừng để việc gấp lấn át việc quan trọng. Cả nhà thử áp dụng xem sao? 🌿`, quote: 'Ưu tiên đúng, kết quả đúng', cursive_quote: 'Tập trung làm đúng việc để', highlight_quote: 'ƯU TIÊN VIỆC QUAN TRỌNG' },
          { post: `🌸 Trong ${purposeLabel}, nhiều người chỉ lên kế hoạch mà quên hành động.\n\nEm thấy rằng: 1 hành động nhỏ hôm nay > 10 kế hoạch hoàn hảo trên giấy.\n\nCả nhà hôm nay đã hành động gì chưa? 💪`, quote: 'Hành động hơn lời nói', cursive_quote: 'Bắt tay vào làm ngay để', highlight_quote: 'HÀNH ĐỘNG NGAY HÔM NAY' },
          { post: `🌿 Sai lầm lớn nhất khi theo đuổi ${purposeLabel}?\n\nĐó là cố làm mọi thứ cùng lúc. Hãy chọn 1 việc — làm cho tốt — rồi mới chuyển sang việc tiếp theo.\n\nCả nhà đang tập trung vào 1 việc duy nhất nào? ✨`, quote: 'Tập trung là sức mạnh', cursive_quote: 'Làm ít nhưng làm sâu để', highlight_quote: 'TẬP TRUNG LÀ SỨC MẠNH' }
        ];
        return fallbacks[new Date().getDate() % fallbacks.length];
      } else if (timeOfDay.includes('trưa') || timeOfDay.includes('12')) {
        const fallbacks = [
          { post: `☕️ Nửa ngày đã trôi qua — cả nhà đã hoàn thành được bao nhiêu % kế hoạch trong ${purposeLabel}?\n\nNếu chưa đạt, đừng lo. Buổi chiều vẫn còn cơ hội.\n\nĐiều quan trọng là KHÔNG DỪNG LẠI 🌿`, quote: 'Buổi chiều vẫn còn cơ hội', cursive_quote: 'Không ngừng nỗ lực để', highlight_quote: 'KHÔNG BAO GIỜ DỪNG LẠI' },
          { post: `🌸 Mẹo nhỏ trong ${purposeLabel} mà em hay áp dụng:\n\nSau bữa trưa, dành 10 phút ghi lại 1 bài học từ buổi sáng. Kiến thức chỉ thành của mình khi được viết ra.\n\nCả nhà thử xem? ✨`, quote: 'Ghi chép là cách học nhanh nhất', cursive_quote: 'Viết ra để nhớ mãi và', highlight_quote: 'GHI CHÉP LÀ SỨC MẠNH' },
          { post: `🌿 Giữa ngày bận rộn, đừng quên nghỉ ngơi đúng lúc.\n\nTrong ${purposeLabel}, sức bền quan trọng hơn tốc độ. Người đi xa không phải người chạy nhanh nhất.\n\nCả nhà có đang biết cách nghỉ đúng cách không? ☕️`, quote: 'Sức bền hơn tốc độ', cursive_quote: 'Nghỉ ngơi đúng lúc để', highlight_quote: 'ĐI XA HƠN MỖI NGÀY' },
          { post: `✨ Buổi trưa — thời điểm tốt để tự hỏi:\n\n"Trong ${purposeLabel}, điều gì đang cản trở mình nhất?"\n\nXác định rào cản = đã giải quyết được 50% vấn đề.\n\nCả nhà thấy rào cản lớn nhất của mình là gì? 🎯`, quote: 'Nhận diện vấn đề là bước đầu giải quyết', cursive_quote: 'Dám đối diện để', highlight_quote: 'VƯỢT QUA RÀO CẢN' },
          { post: `🌸 Một thực tế trong ${purposeLabel}:\n\nKhông ai thành công một mình. Hãy tìm cộng đồng, tìm mentor, tìm người đồng hành.\n\nCả nhà đã có nhóm đồng hành cho mình chưa? 🌿`, quote: 'Đồng hành để đi xa hơn', cursive_quote: 'Kết nối đúng người để', highlight_quote: 'ĐỒNG HÀNH ĐI XA HƠN' }
        ];
        return fallbacks[new Date().getDate() % fallbacks.length];
      } else {
        const fallbacks = [
          { post: `🌙 Kết thúc một ngày trong ${purposeLabel} — hãy tự hỏi:\n\n"Hôm nay mình đã tiến bộ hơn hôm qua ở điểm nào?"\n\nDù chỉ 1% cũng đáng tự hào. Cả nhà hôm nay đã làm được gì? ✨`, quote: 'Mỗi ngày tiến bộ 1%', cursive_quote: 'Nhìn lại để tiến về phía', highlight_quote: 'TIẾN BỘ MỖI NGÀY' },
          { post: `✨ Trước khi nghỉ ngơi, em muốn nhắc cả nhà:\n\nThất bại hôm nay trong ${purposeLabel} không phải là kết thúc — đó là bài học cho ngày mai.\n\nViết ra 1 điều rút ra được hôm nay nhé 🌿`, quote: 'Thất bại là bài học quý giá', cursive_quote: 'Rút kinh nghiệm để', highlight_quote: 'HỌC TỪ THẤT BẠI' },
          { post: `🌿 Cuối ngày rồi — đừng mang áp lực ${purposeLabel} lên giường.\n\nNghỉ ngơi cũng là một phần của thành công. Não bộ cần thời gian để sắp xếp lại mọi thứ.\n\nCả nhà đã có thói quen "tắt công việc" buổi tối chưa? 🌙`, quote: 'Nghỉ ngơi là một phần của thành công', cursive_quote: 'Thư giãn để ngày mai', highlight_quote: 'NGHỈ NGƠI ĐÚNG CÁCH' },
          { post: `🌸 Em tin rằng: những người kiên trì trong ${purposeLabel} sẽ nhìn thấy kết quả.\n\nKhông phải ngày mai, có thể tuần sau, tháng sau. Nhưng CHẮC CHẮN sẽ đến.\n\nCả nhà có đang giữ vững niềm tin không? 💪`, quote: 'Kiên trì sẽ thấy kết quả', cursive_quote: 'Giữ vững niềm tin để', highlight_quote: 'KIÊN TRÌ KHÔNG BỎ CUỘC' },
          { post: `💫 Một ngày nữa khép lại. Dù kết quả trong ${purposeLabel} hôm nay thế nào, hãy biết ơn vì mình đã cố gắng.\n\nNgày mai lại là cơ hội mới. Nghỉ ngơi thật tốt nhé cả nhà 🌙`, quote: 'Biết ơn và tiếp tục tiến bước', cursive_quote: 'Biết ơn hôm nay để', highlight_quote: 'NGÀY MAI TỐT ĐẸP HƠN' }
        ];
        return fallbacks[new Date().getDate() % fallbacks.length];
      }
    }
