...
class DataStore {
    ...
    for (const userId of Object.keys(this.users)) {
      if (this.users[userId].greetingStep === undefined) {
        ...
        if (sent.some(m => m.content.includes('Thông tin lớp học bên dưới') || m.content.includes('Link học bên dưới'))) {
          step = 3;
        } else if (sent.some(m => m.content.includes('muốn tìm hiểu thêm thông tin gì cứ nhắn Bích nhé'))) {
          step = 2;
        } else if (sent.some(m => m.content.includes('Rất biết ơn'))) {
          step = 1;
        }
  ...
  upsertUser(userId, displayName = null) {
    if (!this.users[userId]) {
      ...
      if (sent.some(m => m.content.includes('Thông tin lớp học bên dưới') || m.content.includes('Link học bên dưới'))) {
        ...
        step = 2;
      } else if (sent.some(m => m.content.includes('Em là trợ lý của cô Lưu Bích'))) {
        step = 1;
      }
  ...
  // ── Group Approval Management (Skill 04) ───────────────

  approveGroup(groupId, name = 'Group Chat', purpose = 'Chia sẻ kiến thức về Phát triển bản thân, Nhân hiệu cá nhân và Ứng dụng AI cùng cô Lưu Bích.') {
    ...
    const groupData = {
      ...
      purpose: purpose || 'Chia sẻ kiến thức về Phát triển bản thân, Nhân hiệu cá nhân và Ứng dụng AI cùng cô Lưu Bích.',
    ...
    if (index === -1) {
      ...
      groupData.purpose = purpose || oldPurpose || 'Chia sẻ kiến thức về Phát triển bản thân, Nhân hiệu cá nhân và Ứng dụng AI cùng cô Lưu Bích.';
  ...
  getApprovedGroup(groupId) {
    ...
    if (typeof group === 'string') {
      return { id: group, name: 'Group Chat', purpose: 'Chia sẻ kiến thức về Phát triển bản thân, Nhân hiệu cá nhân và Ứng dụng AI cùng cô Lưu Bích.' };
    }
    return group;
  }
  ...
  getApprovedGroups() {
    ...
    return this.approvedGroups.map(g => {
      if (typeof g === 'string') {
        return { id: g, name: 'Group Chat', purpose: 'Chia sẻ kiến thức về Phát triển bản thân, Nhân hiệu cá nhân và Ứng dụng AI cùng cô Lưu Bích.' };
      }
      return g;
    });
  }
  ...
  getLastScenarioId() {
    return this.stats.lastScenarioId || 0;
  }

  setLastScenarioId(id) {
    this.stats.lastScenarioId = id;
    this._saveAll();
  }
