...
async def check_ssv():
    ...
    try:
        ...
        if df.empty:
            print("❌ Failed to fetch SSV data.")
            return
        ...
        for idx, row in recent.iterrows():
            ...
            has_sweep = sub_df['sweep_low'].iloc[-45:].any() # swing lookback 45 candles
            
            if row.get('volume_ratio', 0) > 1.2 or signal:
                status_str = "🟢 MATCHED" if signal else "❌ REJECTED"
                print(f"\nTime: {idx} | Close: {row['close']:.4f} | {status_str}")
                print(f"  EMA 34: {ema34:.4f} | EMA 89: {ema_89:.4f} | EMA 200: {ema_200:.4f} | EMA 610: {ema_610:.4f}")
                ...
                if signal:
                    ...
                    print(f"    * Checklist: {signal.checklist}")
                    print(f"    * Checklist detail: {signal.volume_detail}")
