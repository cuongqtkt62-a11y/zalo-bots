...
async def check():
    ...
    print("--- DEBUGGING BTC 5M SIGNAL CONDITIONS ---")
    try:
        # Load markets
        ...
        if df_daily.empty:
            print("Daily DF empty")
        else:
            d_last = df_daily.iloc[-1]
            print(f"Daily Close: {d_last['close']:.2f}")
            print(f"Daily EMA89: {d_last.get('ema_89', 0):.2f} | EMA200: {d_last.get('ema_200', 0):.2f} | EMA610: {d_last.get('ema_610', 0):.2f}")
            print(f"Daily partial_bullish: {d_last.get('partial_bullish_order', False)}")
            print(f"Daily partial_bearish: {d_last.get('partial_bearish_order', False)}")
            
        if df_h4.empty:
            print("H4 DF empty")
        else:
            h4_last = df_h4.iloc[-1]
            print(f"H4 Close: {h4_last['close']:.2f}")
            print(f"H4 EMA89: {h4_last.get('ema_89', 0):.2f} | EMA200: {h4_last.get('ema_200', 0):.2f} | EMA610: {h4_last.get('ema_610', 0):.2f}")
            print(f"H4 partial_bullish: {h4_last.get('partial_bullish_order', False)}")
            print(f"H4 partial_bearish: {h4_last.get('partial_bearish_order', False)}")
        ...
        print(f"Price Distance Abs: {price_distance_abs:.2f} | 2.0 * ATR: {2.0 * last.get('atr', 0):.2f}")
