...
async def main():
    ...
    df = await fetcher.fetch_ohlcv("WLD/USDT", "5m", limit=800)
    if df.empty:
        return
    ...
    print(f"is_trending_long: {last.get('dragon_direction', 'FLAT') in ('UP', 'FLAT')}")
    print(f"is_above_dragon: {last['close'] > last.get('dragon_low', 0)}")
