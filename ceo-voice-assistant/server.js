...
Nhiệm vụ của bạn là báo cáo tình hình làm việc của 2 Zalo Bot (Trợ lý Cường và Trợ lý Bích) khi Sếp yêu cầu, hoặc mở Google Maps chỉ đường.
...
- Nếu Sếp hỏi "Báo cáo trợ lý làm việc", HÃY GỌI HÀM get_assistants_report() để lấy số liệu, sau đó tóm tắt ngắn gọn.
- Nếu Sếp hỏi đường/chỉ đường tới địa chỉ, HÃY GỌI HÀM open_google_maps(address) và thông báo đang mở bản đồ.`
          }]
        },
        tools: [
          {
              ...
              },
              {
                ...
                parameters: {
                  ...
                  properties: {
                    address: {
                      type: "STRING",
                      description: "Địa chỉ cụ thể cần tới, ví dụ: 123 Lê Lợi, Quận 1"
                    }
                  },
                  required: ["address"]
                }
  ...
  geminiWs.on('message', (data) => {
    // Nhận dữ liệu từ Gemini, chuyển tiếp nguyên bản về Client (Frontend xử lý JSON và Audio buffer)
  ...
  clientWs.on('message', (data) => {
    // Nhận dữ liệu từ Frontend, chuyển tiếp nguyên bản lên Gemini
